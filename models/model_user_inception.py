# Wrapper for user's InceptionTime model file.
import importlib.util, os, random, datetime
HERE = os.path.dirname(__file__)

def _load_user_module():
    path = os.path.join(HERE, "originals", "inceptiontime-xauusd.py")
    if not os.path.exists(path):
        return None
    spec = importlib.util.spec_from_file_location("user_inception", path)
...
def _fallback(symbol: str, sample_size: int, horizon_days: int, random_seed: int | None):
    # Slight positive drift to mimic learned trend
    if random_seed is not None:
        random.seed(random_seed + 11)
    base = 2400.0 if symbol == "XAUUSD" else 1.28
    vol = (30.0 if symbol == "XAUUSD" else 0.005) / (sample_size ** 0.5)
    drift = 0.0004 if symbol == "XAUUSD" else 0.0001
